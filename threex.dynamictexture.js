...
THREEx.DynamicTexture.prototype.drawPixel = function(x,y, color, size){
...
THREEx.DynamicTexture.prototype.eraser = function(x,y, size){
	this.context.globalCompositeOperation = "source-over";
	if (size <= 1 && size > 0){
		this.context.clearRect(x , y , size,size);
	}else{
		this.context.clearRect(x - (size / 2), y - (size / 2), size,size);
	}
	this.texture.needsUpdate	= true;
...
THREEx.DynamicTexture.prototype.fillPixels = function(x,y, color){
	this.context.globalCompositeOperation = "source-over";
	floodFill(this.context,x,y, color)
	this.texture.needsUpdate	= true;
}


THREEx.DynamicTexture.prototype.drawWithGradient = function(x,y, color){
	this.context.globalCompositeOperation = "source-over";
	var grd = this.context.createLinearGradient(0, 0, 0, this.canvas.width / 2);
...
THREEx.DynamicTexture.prototype.colorize = function(x,y, color){
...
THREEx.DynamicTexture.prototype.reset = function(){
